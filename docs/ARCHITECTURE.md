...
- **`music_rater_albums` corpus** -- a per-user table (`src/db/schema.ts`) populated by a nightly sync (`syncMusicRaterCorpus`, `src/core/music-rater/sync.ts`, job type `music_rater_sync`) that pages a connected music-rater instance's scored albums via `listScoredAlbums` and upserts them keyed on `(userId, musicRaterAlbumId)`. It is the substrate for two independent, otherwise-unconnected features: the `critically-acclaimed` discovery mode (`src/core/discovery-modes/modes/critically-acclaimed.ts`), which reads `getUnresolvedAcclaimedAlbums` for un-owned, high-scoring rows and resolves each to a MusicBrainz artist + release group exactly once, stamping `resolvedAt` (ascending-nulls-first cursor, same idiom as library gap-fill) so a permanently unmatchable row cannot starve the rest of the corpus; and the `criticScore` album signal folded into `applyAlbumModifier` above, which looks up a candidate's rating with `findMusicRaterScoresByNames` keyed by `criticScoreKey` (normalized `artist::title`, both sides of the comparison built by the one function). Neither path needs the other's resolution: the scoring lookup matches on normalized name/title regardless of whether the row has been resolved to an MBID yet. A user with no music-rater connection has an empty table, so both reads are no-ops -- the mode reports no candidates and the score map is empty.
