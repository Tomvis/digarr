...
function musicbrainzConfig(): {
  musicbrainzUrl: string
  musicbrainzIntervalMs: number
  musicbrainzIsPublic: boolean
} {
  ...
  return {
    ...
    // Whether we are talking to MusicBrainz's shared public service, as opposed
    // to a self-hosted mirror. The rate governor in
    // `src/core/clients/musicbrainz.ts` only owes a budget to the former.
    musicbrainzIsPublic: hostname === 'musicbrainz.org' || hostname.endsWith('.musicbrainz.org'),
  }
}
